 ...
 * @return bool result
 */
function xmldb_local_hypercron_upgrade($oldversion) {
    global $DB, $USER, $SITE;
    ...
    if ($result && $oldversion < 2019000007) {
        if (!$dbman->table_exists('hypercron_logs')) {
            $dbman->install_one_table_from_xmldb_file(__DIR__.'/install.xml', 'hypercron_logs');
        }
    ...
    if ($oldversion < 2019000008) {
        ...
        $dbman = $DB->get_manager();
        $table = new xmldb_table('hypercron_logs');
